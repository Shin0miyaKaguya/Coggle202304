...
for n in [100,200]:
	...
	pca.fit(source_feat)
	X_t = pca.transform(source_feat)
	Y_t = pca.transform(query_feat)
	sims = np.dot(Y_t, X_t.T)
	res = np.argmax(sims, axis=1)
	...
	submit.to_csv("submit/submit_task6_PCA{}.csv".format(n), index=False)
